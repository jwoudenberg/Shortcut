...
import String
import Svg exposing (Attribute, Svg, svg, g, path)
...
-- This is a repetition of a piece of logic in elm-css-helpers.
-- The version implemented there only works with the Html class attribute.


namespacedClass : List class -> Attribute msg
namespacedClass list =
    list
        |> List.map (identifierToString shortcutNamespace.name)
        |> String.join " "
        |> class
    ...
    case model of
        UTurn ->
            svg
                ...
                [ g
                    [ namespacedClass [ PathContainer ]
                    , transform (transformString mirrored rotation)
                    ]
                    [ path
                        [ namespacedClass [ PathFill ]
