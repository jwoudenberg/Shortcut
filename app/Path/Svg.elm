module Path.Svg (Shape(..), Mirrored, Rotation, svgElement) where
...
svgElement : Shape -> Mirrored -> Rotation -> Svg
svgElement shape mirrored rotation =
    case shape of
        UTurn ->
            svg
                [ version "1.1"
...
transformString : Mirrored -> Rotation -> String
transformString mirrored rotation =
    let
        mirrorString = "scale(-1 1) translate(-750, 0)"

        turnString : Int -> String
        turnString rotation = "rotate(" ++ toString (rotation * 90) ++ " 375 375)"
    in
        if mirrored then
